...
def train_task(task_num):
    ...
    if task_num == 1:
        ...
        reg_sets = ['/Benchmark_supplementary/mid_scale_benchmarks/4tasks_{}/B{}_train.cvs'.format(split, task_num - 1)]

        if trainval:
            i = task_num - 1
            while i > 0:
                reg_sets.append('/Benchmark_supplementary/mid_scale_benchmarks/4tasks_{}/B{}_test.cvs'.format(split, i))
                i -= 1
        # '/data_splits/4tasks/B1_test.cvs']
